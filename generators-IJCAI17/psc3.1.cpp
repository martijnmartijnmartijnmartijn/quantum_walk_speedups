...
vector<double> angleVar;        // Vector representing the angle in [0,2PI) por variables and clauses
vector<double> angleCla;                     
vector<vector<int> > neighs;    // Vector representing neighbors of a clause node
int edges = 0;                  // Edges already created
vector<vector<double> >P;       // P[i][j] = un-normalized probabiliti of edge i<->j (or distance between nodes if T=0)
double SP = 0;                  // Sum of the un-normalized probabilities
...
inline bool checkEdge(int i, int j){
...
void createEdge(int i, int j){
  //cerr << "c New edge "<<i<<" - "<<j<<" P="<<P[i][j]<<endl;
...
void printGraph(){
  ...
  if (!varRename) {
    ...
    getorder(angleVar,indvar,true);
    getorder(angleCla,indcla,false);
...
int main(int argc, char** argv){
  ...
  // Compute (un-normalized) probabilities (or distance for T==0)
  P.resize(n+1);
  for (int i=1; i<=n; i++) {
    P[i].resize(m+1);
    for (int j=1; j<=m; j++) {
      double diffang = M_PI - myabs(M_PI - myabs(angleCla[j] - angleVar[i]));
      P[i][j] =  pow(i,b) * pow(j,B) * diffang;
      if (T>0) {
	P[i][j] = 1.0 / pow(P[i][j], 1.0/T);
	SP += P[i][j];
      }      
    }
  }
  ...
  if (K>0) {
	...
	for (int i=1; i<=n; i++) {
	  if (selected.size() < K)
	    insertOrdered(selected, make_pair(i,P[i][j]));
	  else {
	    if(P[i][j] < selected[selected.size()-1].second){
	      insertOrdered(selected, make_pair(i,P[i][j]));
	      selected.pop_back();
	    }
	  }
	...
	} while (eold < e); //SPnew=SPOld and e=eold, therefore finish iteration
	
	// Starts random generation of edges
	//for (int i=1; i<=n; i++) {
	while (e < K) {
	  int i = rand()%n+1;
	  double prob = P[i][j]*(K-eold)/SPold;
  ...
  if (k>0) {
	...
	for (int j=1; j<=m; j++) {
	  if (!checkEdge(i,j)) {
	    if (selected.size() < k*m)
	      insertOrdered(selected, make_pair(make_pair(i,j), P[i][j]));
	    else {
	      if(P[i][j] < selected[selected.size()-1].second){
		insertOrdered(selected, make_pair(make_pair(i,j), P[i][j]));
		selected.pop_back();
	...
	for (int i=1; i<=n; i++)
	  for (int j=1; j<=m; j++) {
	    if (!checkEdge(i,j) && P[i][j]*(k*m-eold)/SPold >= 1) // If prob needs being truncated, generate corresponding edge
	...
	int i=rand()%n+1;
	int j=rand()%m+1;
	double prob = P[i][j] * (k * m - eold) / SPold;
	if( !checkEdge(i,j) && ((double)rand() / (double)RAND_MAX) < prob)
